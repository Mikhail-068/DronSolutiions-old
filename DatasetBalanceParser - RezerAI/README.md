...
На графике показана выборка, содержащая все аннотации с классом class_0, который подлежит удалению из датасета, и объекты других классов, которые попутно отобрались вместе со всеми файлами аннотаций включающими class_0:

![](./DocsImg/class_0_remove.png)

На графике показана статистика датасета, после удаления из него всех файлов аннотаций, содержащих удаляемый из датасета класс class_0:
...
Для данного датасета данный дисбаланс этого мажорного класса перманентный и будет повторяться при каждом последующем отборе из этого датасета выборки в данном случае либо с порогом в 500 либо с любым другим порогом, но все остальные количества объектов по другим классам будут численно стремится к пороговому значению, но не менее его.
